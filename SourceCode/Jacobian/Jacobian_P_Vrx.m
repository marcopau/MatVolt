...
for i = 1:nmeas
    m = from(i);
    n = to(i);
    Hr(i,m) = -G(m,n)*(2*V.real(m)-V.real(n)) - B(m,n)*V.imag(n) + 2*Gs(m,n)*V.real(m);   % derivative dP/dVr(m)
    Hr(i,n) = G(m,n)*V.real(m) + B(m,n)*V.imag(m);                                        % derivative dP/dVr(n)
    Hx(i,m) = -G(m,n)*(2*V.imag(m)-V.imag(n)) + B(m,n)*V.real(n) + 2*Gs(m,n)*V.imag(m);   % derivative dP/dVx(m)
    Hx(i,n) = G(m,n)*V.imag(m) - B(m,n)*V.real(m);                                        % derivative dP/dVx(n)
end
