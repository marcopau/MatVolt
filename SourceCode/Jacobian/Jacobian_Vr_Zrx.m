function H = Jacobian_Vr_Zrx(Znod, bus, I)
% This function returns the Jacobian for real voltages (their derivatives) 
%  with respect to branch impedance variables.
%
% SYNTAX:
%   H = Jacobian_Vr_Zrx(Znod, bus, I)
%
% INPUTS: 
...
%
% OUTPUTS:
%   H : Jacobian matrix of real voltages with respect to branch impedances
...
for i = 1:nmeas
    m = bus(i);
    idx = find(Znod(m,:));
    HZr(i,idx) = -I.real(idx);          % derivative dVr/dZr
    HZx(i,idx) = I.imag(idx);           % derivative dVr/dZx
end
