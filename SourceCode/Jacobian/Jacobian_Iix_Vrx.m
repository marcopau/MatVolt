function H = Jacobian_Iix_Vrx(node, Y, bus, type)
% This function returns the Jacobian for imaginary current injections
%  their derivatives) with respect to rectangular voltage variables.
%
% SYNTAX:
%   H = Jacobian_Iix_Vrx(node, Y, bus, type)
%
% INPUTS: 
...
for i = 1:nmeas
    m = bus(i);
    Hr(i,:) = Y.imag(m,:);      % derivative dIx/dVr
    Hx(i,:) = Y.real(m,:);      % derivative dIx/dVx
end
