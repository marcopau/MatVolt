function [resSE, num_iter, unc] = SE_WLS_BC_vm(branch, node, zdata, admittance)
...
%  rectangular branch currents as state variables + virtual measurements
%  for zero injections and other equality constraints.
%
% SYNTAX:
%   [resSE, num_iter] = SE_WLS_BC_vm(branch, node, zdata, admittance)
% 
% INPUTS:
...
while bdata_id == 1
    ...
    I = Flat_start_current(branch.num);
    if ~isempty(bussyncv)                                                  % detect if there are PMU measurements in input
        SEtype = 2;                                                        % estimator with PMUs
        StateV = [V.real(1); V.imag(1)];
    else 
        SEtype = 1;                                                        % estimator without PMUs
        StateV = V.mag(1);
    end
    Znod = Znod_creation(branch, node);
    State = [StateV; I.real; I.imag];                                      % definition state vector for WLS
    ...
    W6 = diag(1./zvar(ifi));                                               % Weights current magnitudes
    ...
    H2 = Jacobian_Iir_Irx(branch, buspi, SEtype);                          % Jacobian active pwr injections
    H3 = Jacobian_Iix_Irx(branch, buspi, SEtype);                          % Jacobian reactive pwr injections
    H4 = Jacobian_Ifr_Irx(branch, brpf, SEtype);                           % Jacobian active branch powers
    H5 = Jacobian_Ifx_Irx(branch, brpf, SEtype);                           % Jacobian reactive branch powers
    H7 = Jacobian_Vr_Irx(branch, Znod, bussyncv, SEtype);                  % Jacobian PMU real voltages
    H8 = Jacobian_Vx_Irx(branch, Znod, bussyncv, SEtype);                  % Jacobian PMU imaginary voltages
    H9 = Jacobian_Iir_Irx(branch, bussyncii, SEtype);                      % Jacobian PMU real current injections
    H10 = Jacobian_Iix_Irx(branch, bussyncii, SEtype);                     % Jacobian PMU imaginary current injections
    H11 = Jacobian_Ifr_Irx(branch, brsyncif, SEtype);                      % Jacobian PMU real branch currents
    H12 = Jacobian_Ifx_Irx(branch, brsyncif, SEtype);                      % Jacobian PMU imaginary branch currents
    ...
    while epsilon > thresh
        ...
        h6 = I.mag(abs(brif));                                             % meas function h(x) for current magnitude meas
        ...
        H1 = Jacobian_V_Irx(branch, Znod, busv, SEtype, V);                % Jacobian voltage magnitudes
        H6 = Jacobian_I_Irx(branch, brif, SEtype, I);                      % Jacobian current magnitudes
        ...
        %%% Update reference voltage and current vector
        if SEtype == 2
            V.complex(1) = complex(State(1),State(2));
        elseif SEtype == 1
            V.complex(1) = State(1);
        end
        I.real = State(1+SEtype:SEtype+branch.num);
        I.imag = State(1+SEtype+branch.num:end);
        I.complex = complex(I.real,I.imag);
        I = Complex_to_all(I);
        V.complex = V.complex(1) - Znod*I.complex;                         % forward sweep to compute all the bus voltages
        V = Complex_to_all(V);
    end
...
cov = inv(H'*W*H);                                                             % covariance matrix state vector
if SEtype == 1
   cov = [cov(1,:); zeros(1,2*branch.num+1); cov(2:end,:)];
   cov = [cov(:,1), zeros(2*branch.num+2,1), cov(:,2:end)];
end
covIb.rx = cov(3:end,3:end);                                                   % covariance matrix rectangular branch currents
...
covV.rx = CovVrx_from_CovIrx(cov, branch, Znod);                               % covariance matrix rectangular voltages
